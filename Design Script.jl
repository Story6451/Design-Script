...
function anode_efficiency(T, P, mdot)
    return T^2 / (2 * mdot * P)
end

function isp(eff, P, mdot)
    Pjet = eff * P
...
Dann_eff = anode_efficiency(Dann_T, P_in, Dann_mdot)
Lee_eff  = anode_efficiency(Lee_T,  P_in, Lee_mdot)
Hyb_eff  = anode_efficiency(Hyb_T,  P_in, Hyb_mdot)
...
Dann_isp = isp(Dann_eff, P_in, Dann_mdot)
Lee_isp  = isp(Lee_eff,  P_in, Lee_mdot)
Hyb_isp  = isp(Hyb_eff,  P_in, Hyb_mdot)
...
bfield = []
if (bfield_file != "")
    bfield = het.load_magnetic_field(bfield_file)
else
    x_bfield = 0:0.0001:(4*L_channel)
    b_field = bfield_model(bmax, g, d, x_p, x_bfield)
